...
-- Give key after purchase
if Config.RequireKeyItem and Config.KeyItemName then
    local Player = QBCore.Functions.GetPlayer(source)
    if Player then
        Player.Functions.AddItem(Config.KeyItemName, 1)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items[Config.KeyItemName], "add")
    end
end
 (license, citizenid, vehicle, hash, mods, plate, garage, state) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
...
QBCore.Commands.Add('transfervehicle', Lang:t('general.command_transfervehicle'), { { name = 'ID', help = Lang:t('general.command_transfervehicle_help') }, { name = 'amount', help = Lang:t('general.command_transfervehicle_amount') } }, false, function(source, args)
    ...
    if target.Functions.GetMoney('cash') > sellAmount then
        ...
        -- Remove key after vehicle is sold
        if Config.RequireKeyItem and Config.KeyItemName then
            player.Functions.RemoveItem(Config.KeyItemName, 1)
            TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[Config.KeyItemName], "remove")
        end
